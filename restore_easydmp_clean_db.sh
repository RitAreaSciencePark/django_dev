...
docker exec -w /app/django/decos_webapp django_dev-webapp-1 python3 manage.py dumpdata --natural-foreign --natural-primary -e contenttypes -e auth.Permission --indent 2 > backups/backup_dump_$timestamp.json
...
docker exec django_dev-db-1 dropdb -U decos 'decos_metadata_db' -f
docker exec django_dev-db-1 createdb -U decos 'decos_webapp_db'
docker exec django_dev-db-1 createdb -U decos 'decos_metadata_db'
printf "Done.\n"
...
docker exec django_dev-db-1 psql -U decos 'decos_webapp_db' -c 'DELETE FROM wagtailcore_site;'
docker exec django_dev-db-1 psql -U decos 'decos_webapp_db' -c 'DELETE FROM wagtailcore_page WHERE id = 2;'
echo "Delete Done."

echo "Loading decos_webapp_db database..."
# LOAD database
ls -oht backups/
echo "insert the name of the database to restore (without .json):"
read database_name
docker exec -w /app/django/decos_webapp django_dev-webapp-1 python3 manage.py loaddata /app/backups/$database_name.json
...
echo "insert the name of the database to restore (without .json):"
read database_name
docker exec -w /app/django/decos_webapp django_dev-webapp-1 python3 manage.py loaddata --database prpmetadata-db /app/backups/$database_name.json
# docker exec -w /app/django/decos_webapp django_dev-webapp-1 python3 manage.py migrate
echo "Loading database Done."